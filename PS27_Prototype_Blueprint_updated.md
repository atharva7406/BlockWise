...
This matches the production stack in the team's Technology Stack Report — the prototype should use the real stack at reduced scope, not throwaway tooling, so the demo and the architecture story are the same system.

- **Backend:** Python 3.12 + FastAPI (async, auto OpenAPI docs — useful if judges ask to see the contract)
- **Data contract:** Pydantic v2 models for every canonical entity (`Asset`, `MaintenanceTask`, `Train`, `BlockWindow`, `SourceMetadata`, `TopologyMap`) — a record that fails validation is quarantined before reaching scoring; this is Gate 0 enforced in code, not just described on a slide
- **Data processing:** Pandas for KM overlap joins, corridor matching, feature prep
- **Database:** PostgreSQL — canonical tasks + bundle certificates + plan/audit rows as relational tables, with the raw canonical payload also kept in a JSONB column for exact audit replay. Add a DB constraint blocking `source_meta.mode = "synthetic"` records from being marked schedulable in a "production" table — cheap to build, and directly demonstrates the mode-tagging safeguard from the stack report
- **Scoring layer:** plain Python (weighted formula), not trained scikit-learn models — skip actual model training unless time allows; a transparent formula is *more* defensible in Q&A than a black-box model with no real training data anyway. Frame it in the demo as "the same interface scikit-learn models (M1–M5) would plug into"
- **Scheduling:** Google OR-Tools CP-SAT — deterministic, seed-reproducible, returns a solver status (OPTIMAL/FEASIBLE/INFEASIBLE) you can show directly on screen
- **Frontend:** React / Next.js + a charting lib (Recharts) for the KM overlap timeline and priority breakdown
- **Data generation:** a Python script producing realistic messy synthetic records (see §6)
- **Deployment:** Docker Compose — one container per service (adapters, scoring, Auto-Shadow, CP-SAT worker, PostgreSQL, frontend), single-command demo spin-up, and it's the same shape you'd hand to a review team later without rearchitecting
...
- **"Why this stack, not an LLM or a simpler heuristic?"** → CP-SAT gives a mathematical feasibility guarantee for train-safety constraints; a probabilistic model can't. Every layer (FastAPI, Pydantic, Pandas, OR-Tools, PostgreSQL, React) is open-source and independently swappable — no vendor lock-in, and the same containers scale from laptop demo → pilot → production without a rewrite.
